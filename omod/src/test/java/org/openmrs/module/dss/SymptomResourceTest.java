...
public class SymptomResourceTest extends BaseSymptomResourceTest<Symptom, SymptomResource> {
	...
	// 	when(service.saveOrUpdate().thenReturn(getObject());
	
	// 	Symptom newlyCreatedObject = getResource().save(getObject());
	// 	assertThat(newlyCreatedObject, notNullValue());
	// 	assertThat(newlyCreatedObject.getUuid(), is(SYMPTOM_UUID));
	// }
	...
	// @Test
	// public void shouldGetAllSymptoms() {
	// 	Symptom mockSymptom = mock(Symptom.class);
	
	// 	when(service.retriveAll(mockSymptom)).thenReturn(Collections.singletonList(mockSymptom));
	// 	Collection<Symptom> result = service.retriveAll(mockSymptom);
	// 	assertThat(result, hasSize(1));
	// 	assertThat(result, hasItem(mockSymptom));
